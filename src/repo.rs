...
pub const DEFAULT_REPO_SUBDIR: &str = "skills";
...
///
/// When `no_suggest` is false and the config allows it, follows `[[suggest]]`
/// entries from loaded repos to discover additional skill sources.
///
...
mod tests {
    ...
    fn test_repo_id_local() {
        let path = PathBuf::from("/home/user/my-repo");
        let id = repo_id(&path, &[]);
        assert_eq!(id, "local:/home/user/my-repo");
    }
